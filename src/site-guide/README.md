...
资源导航这类“所有内容都在一页”的页面，可以在 `sidebar.ts` 中手写锚点链接，例如 `/resources/#官方文档`。这样左侧栏看起来像一级目录，点击时仍然停留在同一个页面内跳转。`src/resources/README.md` 新增、删除或改名一级 `##` 分类时，必须同步更新 `/resources/` 的侧边栏锚点；此类页面可以在 frontmatter 中设置 `toc: false`，避免右侧再出现一份重复的“此页内容”。
...
教程应当是一篇可以独立阅读的文章：读者不需要知道它来自什么需求、谁提出过什么问题，也能理解它要解决的问题、核心概念、示例和结论。

- 内容只围绕教程主题展开。需求沟通、写作计划、个人背景和对话中的问答，只能帮助确定选题与范围，不能整理进正文。
- 实质性教程在开头设置 `## 前言`，说明学习背景、问题或适用场景；在末尾设置 `## 总结`，回收核心结论和使用边界。
- 示例应服务于概念解释。涉及工程实践时，优先使用可运行、贴近真实工作的场景，并说明必要的前提和限制。
- 技术术语应使用准确且稳定的叫法。遇到版本、协议、框架行为等容易变化的内容，应以官方文档、规范或项目原始文档为准。
- `## 参考资料` 按实际需要添加：只列出确实用于核对或支撑文章的资料，优先官方文档、规范和项目文档；不要为了凑格式添加无关链接。
