...
- [GORM 官方中文文档](https://gorm.io/zh_CN/docs/)

GORM 是 Go 生态中常用的 ORM（Object-Relational Mapping，对象关系映射）库。它可以把 Go 结构体映射为数据库表，并通过 Go API 完成数据的创建、查询、更新和删除。

学习 GORM 的重点不是记住所有方法，而是掌握下面这条主线：

1. 建立并复用数据库连接；
2. 使用结构体定义数据模型；
3. 理解 GORM 如何生成 SQL；
4. 掌握常见 CRUD、条件查询和分页；
5. 正确处理零值、错误、事务和 SQL 注入。

GORM 还提供关联关系、钩子、迁移器、优化器提示等大量功能，但这些内容应在项目确实需要时再查阅官方文档，不必在入门阶段逐项学习。

> GORM 目前同时提供传统 API 和泛型 API。本节统一使用应用广泛的传统 API，避免在同一节中混用两套写法。掌握核心概念后，再阅读官方的泛型 API 文档即可。
...
#### 3. 请求链路中使用 `WithContext`

这样才能让超时、取消和链路追踪作用于数据库操作。

#### 4. 开发环境观察 SQL，生产环境控制日志量
...
原文的 GORM 安装、模型、迁移、CRUD、事务、关联与原生 SQL 内容已经保持原顺序复制。GORM 的价值在于组织模型和 SQL 构造，底层仍由 `database/sql` 和 MySQL 驱动完成连接与执行。无论采用 ORM 还是原生 SQL，连接池、参数化、错误处理、索引、事务边界和并发控制仍然需要由应用明确负责。
