...
本文从 MySQL 表和 SQL 基础出发，依次进入参数化查询、CRUD、事务、连接池和 `sqlx`。读完后，你应当能够判断：什么时候直接使用标准库最清晰，什么时候引入 `sqlx` 更合适，以及无论选择哪一个都不能忽略哪些资源和一致性边界。
...
`database/sql`、MySQL 驱动和 `sqlx` 分别解决“统一访问模型”“与 MySQL 通信”“减少样板代码”三个问题。`sqlx` 不会创建独立的事务与连接池，也不会替应用处理 SQL 安全、索引或并发控制。

一次可靠的数据访问至少要守住四条边界：连接池在进程内长期复用；所有外部值通过占位符绑定；多行结果集及时关闭、单行查询在 `Scan` 处判断错误；需要共同成功的写入全部经同一个 `tx` 执行，并明确提交或回滚。掌握这些边界后，CRUD API 只是表达业务的工具，而不会成为数据库访问的风险来源。
