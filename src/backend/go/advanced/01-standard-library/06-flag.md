...
命令行参数是程序启动时最直接的配置入口。`flag` 用极少的代码提供默认值、类型转换和帮助信息，足以覆盖脚本、开发工具和服务启动配置等大量场景。关键是先定义清楚参数语义，再在解析后做业务校验。
...
`flag` 将命令行文本解析到带类型的变量中：少量参数可使用返回指针的定义函数，参数较多时适合写入配置结构体。调用 `Parse` 后再读取参数值与位置参数；布尔参数和第一个位置参数会影响解析行为。`flag` 负责语法解析，端口范围、配置组合等业务约束仍应由程序显式验证。
