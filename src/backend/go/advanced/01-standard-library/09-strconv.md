...
配置、环境变量、命令行参数和 HTTP 请求传入的几乎都是字符串，而程序真正需要的是有边界、有类型的值。`strconv` 提供了这层转换；更重要的是，它让格式不合法和数值越界能够被明确地识别和处理。
...
十进制 `string` 与 `int` 的常见互转使用 `Atoi` 与 `Itoa`；其他类型、进制或精度要求使用 `Parse` 和 `Format` 系列。外部文本转换后不能只看 `err == nil`，还要继续验证业务范围。尤其不要用 `string(intValue)` 生成十进制数字文本，它表达的是 Unicode 码点转换。
