...
`strconv` 负责基本类型与字符串之间的可靠转换，也提供转义、引用和类型判断功能。以下按原有顺序保留常用 API、错误处理和使用建议。

> 说明：本章按原有内容、示例和顺序拆分为独立文章。代码中的资源关闭、错误处理、参数含义等关键点均保留在原示例及其紧邻说明中，便于对照阅读。

在读取环境变量、配置项、命令行参数和 HTTP 请求参数时，程序得到的数据通常是字符串。要参与计算或逻辑判断，就需要将字符串转换为整数、浮点数或布尔值；反过来，将这些值写入文本时，也需要转换为字符串。

Go 标准库中的 `strconv` 包用于完成基本类型与字符串之间的转换。虽然该包提供了较多函数，但日常开发主要掌握下面几类即可：

| 使用场景                               | 常用函数                              |
| -------------------------------------- | ------------------------------------- |
| 十进制 `string` 转 `int`               | `strconv.Atoi`                        |
| `int` 转十进制 `string`                | `strconv.Itoa`                        |
| 字符串转布尔值、浮点数或指定范围的整数 | `ParseBool`、`ParseFloat`、`ParseInt` |
| 数值按指定进制或精度转字符串           | `FormatInt`、`FormatFloat`            |

其中，`Atoi` 和 `Itoa` 最常用。只有需要处理 `int64`、浮点数、无符号整数、其他进制或指定精度时，才需要使用 `Parse` 和 `Format` 系列函数。

## 14.6.1 `string` 与 `int` 之间的转换
...
把外部文本转换为数值或布尔值时，必须检查 `Parse` 系列返回的错误，并根据业务范围继续校验。把数值转换为字符串时使用 `Itoa`、`FormatInt`、`FormatFloat`，不要把整数直接转换为 `string`。需要安全嵌入字符串时使用 `Quote` 或 `QuoteToASCII`。
