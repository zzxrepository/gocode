...
`fmt` 是 Go 程序与文本世界之间最常用的接口：它既能把值清晰地呈现到终端、文件或缓冲区，也能把结构化文本读回变量。真正需要掌握的不是记住所有占位符，而是根据数据流向选择合适的函数族，并让格式、错误处理和输入边界保持明确。
...
`fmt` 的使用可以归结为三件事：向何处写、是否需要格式串、是否需要得到字符串。标准输出使用 `Print` 系列，指定目标使用 `Fprint` 系列，构造文本使用 `Sprint` 系列；错误包装时用 `%w` 保留可识别的错误链。面对输入时，接收变量必须传指针，并始终检查扫描结果与错误。
