...
时间代码的难点不在于取得“现在”，而在于准确表达业务含义：一个瞬间、一次持续时间，还是某个时区中的日历时间。`time.Time`、`time.Duration` 和 `time.Location` 分别承担这三种职责；把它们混为一谈，往往就是时区偏移、超时失效和月末计算出错的根源。

`time` 也连接着其他常用包：`flag.Duration` 把命令行中的 `5s` 解析为时长，`context.WithTimeout` 使用 `Duration` 建立取消边界，日志与数据库通常记录 `Time` 或 Unix 时间戳。先确定业务需要的是“绝对瞬间”还是“日历时间”，再选择存储、传输、展示和计算方式，时间逻辑才会稳定。
...
`time.Time` 表示一个瞬间，`time.Duration` 表示固定长度的间隔，`time.Location` 决定这个瞬间以何种日历形式呈现。固定时长使用 `Add`，按月份和年份推进使用 `AddDate`；判断先后使用 `Equal`、`Before`、`After`，不要用 `==` 替代业务上的时间比较。

跨系统传输优先使用包含偏移量的 RFC 3339，解析不带时区的文本时必须明确其所属 `Location`。超时、重试和周期任务本质上都依赖 `Duration`，并应与 `context`、`Timer` 或 `Ticker` 的生命周期配合；创建了可停止的定时器，就应明确何时停止它。
