...
查找是指在一大段文本中找到符合规则的内容。例如在代码仓库里搜索所有以 `.test.js` 结尾的测试文件，或者在日志里搜索所有包含 `ERROR` 的行。校验是指判断一整段输入是否符合规则，例如注册时检查邮箱格式是否大致正确。提取是指从文本中拿出某个部分，例如从 `GET /api/users/42 HTTP/1.1` 中提取路径 `/api/users/42`。替换则是把符合规则的内容改成新格式，例如把日期 `2026/08/07` 批量替换成 `2026-08-07`。
